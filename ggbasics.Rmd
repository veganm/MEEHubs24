...
```{r, echo=FALSE, message=FALSE, results='hide'}
pacman::p_load(tidyverse, RColorBrewer, pokemon, colorspace, dichromat, remotes, cowplot)
```
...
Let's see if there is any relationship between attack/defense components and speed. Notice that we are saving the plot as an object now; see if you can find it in your environment. What is contained in a *ggplot* object? 
```{r}
...
Since these scales are designed to be discrete, let's generate a plot that uses discrete (categorical) data. For example, we might ask how egg types are distributed over different types of Pokemon:
```{r}
...
Much better, except that the colors are fairly unpleasant. Let's use ColorBrewer to access some new options.
```{r}
...
**EXERCISE:** Use these tools to interrogate some of the palettes we have just seen. Make your own palette and do the same.
```{r}
