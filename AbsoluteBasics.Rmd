...
R is smart enough to return an error if you try to pass a non-numeric value to a function that carries out a numeric operation:
```{r}
addto3("apple")
```

But there are plenty of operations that can be done with character strings. One common operation is *concatenation*, which is carried out here by the *paste()* function. Let's see what this function does first, so we have the syntax:
```{r}
?paste
```

Now we can use this function as part of our own function:
```{r}
...
In the empty code block below, create a function that carries out some other simple one-step arithmetic operation.
```{r}
...
We can also *pipe* these commands. Remember that the operator *%>%* "pipes" whatever is on the left-hand side of the "funnel" into the command(s) on the right-hand side. Putting a dot "." in front of the *$* operator here tells R to pass the value from the pipe into a specific location:
```{r}
pokemon %>% .$pokemon[1:10]
```
